...
!
   ...
   enddo
!
   !!$acc parallel loop collapse(2) private(tvcon)
   do k = kts,kte
     do i = its,ite
       tvcon = (1.+ep1*qvx(i,k))
...
!
   !!$acc parallel loop private(tvcon)
   do i = its,ite
     tvcon = (1.+ep1*qvx(i,1))
...
!
   !!$acc parallel loop collapse(2) private(prfac,prfac2,prnumfac,phim8z,prnum0,prnum)
   do k = kts,klpbl
     do i = its,ite
       if(k.lt.kpbl(i)) then
...
!
   !!$acc parallel loop collapse(2) private(ss,govrthv,ri,qmean,tmean,alph,chi,zk,rlamdz,rl2,dk,sri,prnum)
   do k = kts,kte-1
     do i = its,ite
       if(k.ge.kpbl(i)) then
