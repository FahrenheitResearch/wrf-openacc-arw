...
!--- computation of effective radii:
 ! Keep the local work arrays resident across the effect-radii phase.
 !$acc data copyin(t,qc,qi,qs,rho) copyout(re_qc,re_qi,re_qs) create(ni,rqc,rqi,rni,rqs)
 ...
 !$acc parallel loop collapse(2) private(lamdac)
 do k = kts,kte
   do i = its,ite
 ...
 !$acc parallel loop collapse(2) private(diai)
 do k = kts,kte
   do i = its,ite
 ...
 !$acc parallel loop collapse(2) private(supcol,n0sfac,lamdas)
 do k = kts,kte
   do i = its,ite
