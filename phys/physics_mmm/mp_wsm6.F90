...
!input arguments:
 ...
 real(kind=kind_phys),intent(in),dimension(its:,:),optional::     &
                                                             pii
...
!inout arguments:
 ...
 logical,intent(in),optional:: restore_t_to_theta
...
!local variables and arrays:
 ...
 logical:: theta_input, restore_theta
 logical:: has_snow_fields, has_graupel_fields, has_precip_budget, use_fallback_sr
...
!
 idim = ite-its+1
 kdim = kte-kts+1
 theta_input = present(pii)
 restore_theta = theta_input
 if (present(restore_t_to_theta)) restore_theta = restore_t_to_theta
 has_snow_fields = present(snow) .and. present(snowncv)
 has_graupel_fields = present(graupel) .and. present(graupelncv)
 has_precip_budget = present(rainprod2d) .and. present(evapprod2d)
 use_fallback_sr = .not. (has_snow_fields .and. has_graupel_fields)
...
!
 if(.not. use_fallback_sr) then
   ...
   do i = its, ite
     rainncv(i) = 0.
     if(has_snow_fields) snowncv(i) = 0.
     if(has_graupel_fields) graupelncv(i) = 0.
     sr(i) = 0.
...
!  qsat(i,k,2) = fpvs(t(i,k),1,rd,rv,cpv,cliq,cice,xlv0,xls,psat,t0c)
   ...
   do k = kts, kte
     do i = its, ite
       qsat(i,k,1)=psat*exp(log(ttp/t(i,k))*(xa))*exp(xb*(1.-ttp/t(i,k)))
       ...
       if(t(i,k).lt.ttp) then
         qsat(i,k,2)=psat*exp(log(ttp/t(i,k))*(xai))*exp(xbi*(1.-ttp/t(i,k)))
       else
         qsat(i,k,2)=psat*exp(log(ttp/t(i,k))*(xa))*exp(xb*(1.-ttp/t(i,k)))
       endif
...
!---------------------------------------------------------------
   !$acc parallel loop collapse(2) private(xmi,diameter)
   do k = kte, kts, -1
     do i = its, ite
       if(qi(i,k).le.0.) then
         work1c(i,k) = 0.
...
!
   if(.not. use_fallback_sr) then
     !$acc parallel loop private(fallsum,fallsum_qsi,fallsum_qg)
     do i = its, ite
...
!       (T>T0: I->C)
!---------------------------------------------------------------
   !$acc parallel loop collapse(2) private(supcol,xlf,supcolt,pfrzdtc,temp,pfrzdtr)
   do k = kts, kte
     do i = its, ite
       supcol = t0c-t(i,k)
       xlf = xls-xl(i,k)
...
!===============================================================
!
   !$acc parallel loop collapse(2) private(supsat,satdt,coeres)
   do k = kts, kte
     do i = its, ite
...
!
   !$acc parallel loop collapse(2) &
   !$acc& private(supcol,supsat,satdt,ifsat,temp,eacrs,egi,xmi,diameter,vt2i,vt2r,vt2s,vt2g,vt2ave,acrfac,xlf,supice,coeres,xni0,roqi0,qimax,alpha2)
   do k = kts, kte
     do i = its, ite
       supcol = t0c-t(i,k)
       n0sfac(i,k) = max(min(exp(alpha*supcol),n0smax/n0s),1.)
...
!     large scale
!
   !$acc parallel loop collapse(2) private(delta2,delta3,value,source,factor,xlf,xlwork2)
   do k = kts, kte
     do i = its, ite
!
       delta2=0.
...
!
 ...
 if (theta_input .and. restore_theta) then
   !$acc parallel loop collapse(2)
   do k = kts, kte
     do i = its, ite
       t(i,k) = t(i,k)/pii(i,k)
