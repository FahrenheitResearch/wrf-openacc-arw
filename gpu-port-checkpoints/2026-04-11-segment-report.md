...
- The latest retained control-path change is a runtime-level nonhydro tail contract in `frame/module_gpu_runtime.F` plus matching `solve_em.F` call sites, so the active suffix is expressed as one named ownership seam instead of ad hoc halo/finish fence assembly.
- That control-contract refactor passed the short nested MPI/OpenACC witness in `nested-smoke-2021-mpi-short-nonhydro-tail-contract` through `2021-12-30_17:05:00` with invariant checks passing.
- Refreshed remote short validations on the same retained branch held the current bands: `20260412T2055Z-tailcontract-default = 115 s` and `20260412T2055Z-tailcontract-hostfences = 159 s`.
...
- The latest retained BENCH diagnosis now comes from true `MPI + hostfences` nested witnesses rather than the earlier non-MPI proxy. On `nested-smoke-2021-mpi-bench20-r06-advancewsplit`, `d02` averaged `28.06 s` per child step, with `rk_scalar_tend_tim = 14.05 s` (`50.5%` of `solve_tim`) and `advance_w_tim = 9.94 s` (`35.7%`), while `advance_w_rhs_tim` was only `0.15 s`; that sharply reduces the value of further RHS-focused `advance_w` work.
- The latest retained scalar win is a vertical-kernel fusion in `dyn_em/module_advect_em.F` for the positive-definite `h5/v3` path. On `nested-smoke-2021-mpi-bench20-r08-vertfuse`, `d02` improved from the `r06` baseline's `28.06 s` mean / `27.81 s` latest to `27.74 s` mean / `27.60 s` latest, with `rk_scalar_pd_flux_tim` falling from `13.996 s` to `13.729 s`.
- That same retained vertical-fuse change also improved the real short nested witness: `nested-smoke-2021-mpi-run20-edgefuse` reported `d02` timings of `32.09 / 29.51 / 29.31 s` for the first three child steps and `d01 = 186.45 s` at `17:00:12`, while `nested-smoke-2021-mpi-run20-vertfuse` moved those to `28.27 / 28.22 / 29.29 s` and `d01 = 152.85 s` at the same parent timestamp.
- The latest unresolved `advance_w` candidate kept the BENCH split but fused the damping and `ph` tail in `dyn_em/module_small_step_em.F`. On the local true nested witness `nested-smoke-2021-mpi-run20-advancew-tailfuse-rerun`, `d02` improved to `26.14 / 25.81 / 25.83 / 28.71 / 28.61 / 29.18 s` with a `27.11 s` mean and `advance_w_tim = 8.22 s`, but the same run drifted to `d01 = 174.17 s` at `2021-12-30_17:00:24`. Rebuilt remote single-domain controls on that branch were manually stopped after both lanes remained at one `wrfout` beyond `17 minutes`, so that runtime change is not retained in the active public path.
